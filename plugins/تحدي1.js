...
const handler = async (m, {conn, usedPrefix, text}) => {
  ...
  const textquien = `*منشـن الي هتتحـداه🦦*\n\n*—◉ *بهذه الطريقه*\n${usedPrefix}تحدي @${global.suittag}`;
  ...
  const caption = `🎮 *لعبة حجر ورقه مقص* 🎮\n\n—◉ @${m.sender.split`@`[0]} شايـف نفـسو وبيتحـداك تعـال علـم عليه🦦 @${m.mentionedJid[0].split`@`[0]} موافـق عل التحـدي\n◉ اكتب "نعم" للموافقه\n◉ اكتب "لا" للرفض\n`;
  ...
  conn.suit[id] = {
    ...
    status: 'wait',
    waktu: setTimeout(() => {
      if (conn.suit[id]) conn.reply(m.chat, `صـاحبك اتاخـر عيـدو لم يرجـع🐤`, m);
