import fg from 'api-dylux'  
 ...
 if (!args[0]) throw `مثال : \n .fb https://www.facebook.com/share/r/ikRsfxAw6CXFwofx/?mibextid=xfxF2i`  
...
await  conn.sendFile(m.chat, r2es, 'error.mp4', `https://www.instagram.com/shika17sm?igsh=MTM2YTJ5Ynh6dTUwcA==`, m); 
 } catch (err1) { 
 try {   
...
await  conn.sendFile(m.chat, VIDEO, 'error.mp4', `*𝙰𝚀𝚄𝙸 𝙴𝚂𝚃𝙰 𝚂𝚄 𝚅𝙸𝙳𝙴𝙾*`, m)     
 ...
 await conn.sendFile(m.chat, urll, 'error.mp4', 'https://www.instagram.com/shika17sm?igsh=MTM2YTJ5Ynh6dTUwcA==', m)      
 } catch (err3) { 
 try { 
 let res = await fbDownloader(args[0]) 
 for (let result of res.download) { 
 let ur = result.url     
 await conn.sendFile(m.chat, ur, 'error.mp4', 'https://www.instagram.com/shika17sm?igsh=MTM2YTJ5Ynh6dTUwcA==', m)} 
 } catch (err4) { 
 try { 
 let res3 = await fetch(`https://latam-api.vercel.app/api/facebookdl?apikey=brunosobrino&q=${args[0]}`)   
 let json = await res3.json() 
 let url3 = await json.video 
 await conn.sendFile(m.chat, url3, 'error.mp4', 'https://www.instagram.com/shika17sm?igsh=MTM2YTJ5Ynh6dTUwcA==', m)          
 } catch (err5) {     
 try { 
 ...
 throw `حذثت مشكلة أَبلغ عنها صاحب البوت حتى يصلحها \nhttps://www.instagram.com/shika17sm?igsh=MTM2YTJ5Ynh6dTUwcA==` 
 }}}}}}}} 
